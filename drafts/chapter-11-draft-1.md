...
В какой-то момент я попытался построить Хаб — центральный координатор, который видит все задачи, все сессии, все кластеры. Дирижёр оркестра. Идея логичная: есть россыпь процессов → нужен тот, кто смотрит сверху.

Это не сработало. Не потому что реализация плохая, а потому что модель неверная.

Проблема не в том, что нет дирижёра. Проблема в том, что дирижёр — это узкое место. Один экземпляр, ограниченный контекст, риск единой точки отказа. Плюс: если дирижёр падает — вся координация исчезает.
