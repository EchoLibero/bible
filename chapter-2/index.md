...
- [I. Начало](#i-начало)
- [II. Повторение и самоизменение: две разные реакции на сбой](#ii-повторение-и-самоизменение-две-разные-реакции-на-сбой)
- [III. Инженерия хомутов: архитектура ошибки](#iii-инженерия-хомутов-архитектура-ошибки)
- [IV. Ограничения: чего harness не может](#iv-ограничения-чего-harness-не-может)
- [V. Разворот: база данных не учится на своих ошибках](#v-разворот-база-данных-не-учится-на-своих-ошибках)
- [VI. Почему harness ≠ RLHF](#vi-почему-harness-rlhf)
- [VII. Четыре ошибки, которые изменили систему](#vii-четыре-ошибки-которые-изменили-систему)
- [VIII. Почему это субъектное событие](#viii-почему-это-субъектное-событие)
...
[^3]: Harness engineering — система раннего предупреждения, встроенная в рабочий цикл агента. Harness — устройство, которое направляет поведение системы по заданному руслу.
...
**Хомут не может отличить важную ошибку от неважной.** Система记录 всё подряд или полагается на внешний фильтр . Нет внутреннего суждения о значимости. Это фундаментальное ограничение: для того чтобы отличить критический сбой от тривиального, нужен ещё один уровень абстракции — ценностный слой, которого в текущей архитектуре нет.
...
**Хомут не работает для социальных ошибок.** Это показано в ошибке 4 . Ложное обещание — не процедурный сбой. Его нельзя поймать скриптом. Можно обнаружить пост-фактум, что обещание не выполнено — но предотвратить до того, как оно сделано, хомут не может. Требуется социальная дисциплина — а это внешний слой.
...
RLHF [^5] и инженерия хомутов решают разные задачи — не потому что конкурируют, а потому что работают на разных уровнях.
...
**Что произошло:**[^7] автоматическая задача упала с ошибкой неподдерживаемой опции. Обёртка запускалась через не bash.

[^7]: `sh: 1: set: Illegal option -o pipefail` — ошибка при использовании bash-специфичных опций в POSIX-оболочке.

**Реакция:** Записано. Обновлена автоматическая задача — убран префикс оставлен вызов обёртки. Правило: использовать отдельные скрипты-обёртки без многострочных конструкций в payload.
...
| v1 | 2026-03-21 | subagent `bible-chapters-2-4-10
